...
p1_raw <- plotColData(sce.raw.woED, y = "total") +
  ...
  scale_y_log10(limits = c(NA, 50000), labels = scales::comma) +
  theme_classic()

p2_raw <- plotColData(sce.raw.woED, y = "detected") +
  ylab("Total number of\ndetected genes per cell") +
  scale_y_log10(limits = c(NA, 10000), labels = scales::comma) +
  theme_classic()
...
```{r violins_cells-genes_combined, fig.width=6, fig.height=8}
p1_raw <- plotColData(sce.raw.woED, y = "total") +
  ylab("Total number of\ncounts per cell") +
  scale_y_log10(limits = c(NA, 50000)) +
  ggtitle("Raw data") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0, face = "bold", size = 11))  # hjust = 0 for left alignment
...
p1_cb <- plotColData(sce.cb.woED, y = "total") +
  ylab("Total number of\ncounts per cell") +
  scale_y_log10(limits = c(NA, 50000)) +
  ggtitle("Ambient RNA corrected") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0, face = "bold", size = 11))
...
qc_summary_df <- qc_summary_df %>%
  mutate(
    Percent_Cells_Retained = (Num_Cells / Num_Cells[1]) * 100,
    Percent_Genes_Retained = (Num_Genes / Num_Genes[1]) * 100
  )

# reshape data for plotting percentages
qc_plot_percent <- qc_summary_df %>%
  select(Step, Percent_Cells_Retained, Percent_Genes_Retained) %>%
  pivot_longer(
...
# rename 'Type' values for clarity
qc_plot_percent$Type <- recode(
  qc_plot_percent$Type,
  Percent_Cells_Retained = "Cells",
  Percent_Genes_Retained = "Genes"
)

# set 'Step' as a factor with levels in the desired order
qc_plot_percent$Step <- factor(qc_plot_percent$Step, levels = qc_labels)
```
