...
library(showtext) # for latex font
library(latex2exp) # for latex font
```

Setup LaTeX fonts.

```{r setup latex fonts}
# add latex font and enable showtext
font_add("cmu_serif", "/usr/share/fonts/truetype/cmu/cmunrb.ttf")
showtext_auto()
```
...
p <- ggplot(qc_long, aes(x = Sample, y = Percentage, fill = Exclusion_Type)) +
  ...
  geom_segment(
    aes(x = 0.5, xend = 0.5, y = 0, yend = 105), color = "black", linewidth = 0.3
  ) +
